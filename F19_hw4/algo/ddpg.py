...
class EpsilonNormalActionNoise(object):
        ...
        def __init__(self, mu, sigma, epsilon, lb=-1.0, ub=1.0):
        ...
        def __call__(self, action):
                ...
                if np.random.uniform() > self.epsilon:
                        ...
                        return np.random.uniform(self.lb, self.ub, size=action.shape)
...
class DDPG(object):
    ...
    def __init__(self, env, lr_a,lr_c, sigma,data_path,plots_path, outfile_name,device,logger=None):
        ...
        self.action_lb = env.action_space.low[0]
        self.action_ub = env.action_space.high[0]
        
        if np.isinf(self.action_lb):
            self.action_lb = -1e3
...
#               estimated_q_after_arr = []
        ...
        for i in range(num_episodes):
            ...
            while (not done) and step<=200:
                ...
                AddNoise = EpsilonNormalActionNoise(0, self.sigma, epsilon,self.action_lb, self.action_ub)
